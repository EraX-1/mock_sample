...
from src.services.azure_ai_search import INDEX_TYPES, AzureAISearch
...
def _index_file(index: int, file: Blob, total_files: int, index_type: str) -> object:
...
def insert_blob_into_db(blob_list: list[Blob]) -> list[File]:
    ...
    return [
        file_repository.insert_one(
            {
                "blob_name": blob.name,
                "blob_url": AzureBlobStorage().get_blob_url(blob.name),
            }
        )
...
def index_files(files: list[Blob], index_type: str, retry: bool = False):
    ...
    if retry:
        ...
        # multiprocessing 用に引数を準備
        cpu_count = os.cpu_count()
        processes = cpu_count * 2 if cpu_count else 8
        args_list = [
            (index, file, total_files, index_type)
            for index, file in enumerate(target_files)
        ]
